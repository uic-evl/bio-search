...
class Resnet(pl.LightningModule):
    ...
    def training_step(self, batch) -> Dict:
        ...
        return {"loss": loss, "train_preds": preds, "train_trues": y_true}
    ...
    def training_epoch_end(self, outputs):
        ...
        epoch_acc = 100 * torch.sum(y_preds == y_trues.data) / (y_trues.shape[0] * 1.0)
        self.log("train_acc", epoch_acc, on_epoch=True, prog_bar=True, logger=True)
    ...
    def validation_step(self, batch) -> Dict:
        ...
        loss = self.loss(y_hat, y_true)
        _, preds = torch.max(y_hat, dim=1)
        return {"loss": loss, "val_preds": preds, "val_trues": y_true}
    ...
    def validation_epoch_end(self, outputs) -> Dict:
        ...
        epoch_acc = 100 * torch.sum(y_preds == y_trues.data) / (y_trues.shape[0] * 1.0)
        self.log("val_acc", epoch_acc, on_epoch=True, prog_bar=True, logger=True)
