...
def insert_figures_to_db(cursor, schema: str, figures: List[DBFigure]):
    ...
    # pylint: disable=line-too-long
    sql = f"COPY {schema}.figures (name,caption,num_panes,fig_type,doc_id,status,uri,parent_id,width,height,coordinates,last_update_by,owner,migration_key,notes,labels,source,page) FROM STDIN"
    with cursor.copy(sql) as copy:
        for elem in figures:
